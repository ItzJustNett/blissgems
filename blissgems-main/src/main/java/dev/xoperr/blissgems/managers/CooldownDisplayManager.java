...
public class CooldownDisplayManager {
        ...
        displayTask = this.plugin.getServer().getScheduler().runTaskTimer((Plugin)this.plugin, () -> {
            try {
                for (Player player : this.plugin.getServer().getOnlinePlayers()) {
                    GemInfo currentGemInfo = getCurrentGemInfo(player);
                    if (currentGemInfo == null) {
                        continue;
                    }

                    // If holding a base gem, require it to be active
                    if (currentGemInfo.type != null && !this.plugin.getGemManager().hasActiveGem(player)) {
                        continue;
                    }
                    ...
                    if (energy == 0) {
                        ...
                        if (cooldownDisplay != null && !cooldownDisplay.isEmpty()) {
                    ...
                    if (cooldownDisplay != null && !cooldownDisplay.isEmpty()) {
                        player.spigot().sendMessage(ChatMessageType.ACTION_BAR, new net.md_5.bungee.api.chat.TextComponent(cooldownDisplay));
                    }
                }
            } catch (Exception e) {
                this.plugin.getLogger().severe("Exception in CooldownDisplayManager task: " + e.getMessage());
                e.printStackTrace();
            }
        }, 0L, 2L);
    }
    ...
    private String buildAddonCooldownDisplay(Player player, GemInfo gemInfo) {
        ...
        if (entries == null || entries.isEmpty()) {
            return gemColor + "▶ " + (def != null ? def.getDisplayName() : gemInfo.gemId);
        }
